...
namespace GoShared {
    ...
    public class LocationManager : BaseLocationManager {
        ...
        async UniTask StartGPS (float waitTime) {
		...
		async UniTask LateStart(float waitTime)
		{
			await UniTask.WaitForSeconds(waitTime);
		...
		public void move(float x,float y)
		{
			...
			if (cam == null)
			{
				v = current + new Vector3(x, 0, y);
			}
			else
			{
				v = current + 
					cam.transform.forward.ToProjectedVector3xz().normalized * y +
					cam.transform.right.ToProjectedVector3xz().normalized * x;
			}
        ...
        ////UI
        async void showBannerWithText(bool show, string text) {
		...
		private async UniTask Slide(bool show, float time) {
...
//		}
		...
		void changeLocationWASD (){
            ...
            if (cam == null)
            {
                ...
                if (Input.GetKey(KeyCode.W))
                {
                    v = current + cam.transform.forward.ToProjectedVector3xz().normalized * demo_WASDspeed;
                }
                if (Input.GetKey(KeyCode.S))
                {
                    v = current + cam.transform.forward.ToProjectedVector3xz().normalized * -demo_WASDspeed;
                ...
                if (Input.GetKey(KeyCode.A))
                {
                    v = current + cam.transform.right.ToProjectedVector3xz().normalized * -demo_WASDspeed;
                }
                if (Input.GetKey(KeyCode.D))
                {
                    v = current + cam.transform.right.ToProjectedVector3xz().normalized * demo_WASDspeed;
