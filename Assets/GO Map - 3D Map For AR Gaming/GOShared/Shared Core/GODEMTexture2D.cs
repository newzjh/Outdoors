...
//			try {
//
				int xOff =  Mathf.FloorToInt(position.x * texsize.x/resolution.x);
				int zOff =  Mathf.FloorToInt(position.y * texsize.y/resolution.y);


				int finalx = xOff + (int)offset.x;
				int finaly = zOff + (int)offset.y;
                if (finalx >= nwidth)
					finalx = nwidth - 1;
				if (finaly >= nheight)
					finaly = nheight - 1;

				colorFormula = finalx + nwidth * finaly;
...
//			}
		...
		public float getAltitude (Vector3 input, Vector3 tileOrigin) {
			...
			x = texsize.x * x / stepSizeWidth;
			z = texsize.y * z / stepSizeHeight;
...
//
		...
		public float FindAltitudeForCoordinate (Coordinates inputCoordinate, Coordinates tileOrigin) {
			...
			Color32 c32 = calculateColor (new Vector2 (x, z), Vector2.zero, texsize);
