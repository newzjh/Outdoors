...
namespace GoMap
{
	...
	public class GOMap : MonoBehaviour 
	{
		...
		[Regex (@"^(?!\s*$).+", "Please insert your OSM API key"),NonSerialized]public string osm_api_key = "d6qPzaurptRX6n3oM6eX";
		[Regex (@"^(?!\s*$).+", "Please insert your Nextzen API key")] public string nextzen_api_key = "";

		public GOMapType mapType = GOMapType.OSM;
        ...
        [Range(0,10)] public float elevationMultiplier = 1.0f;
		...
		public GOSatelliteType satelliteType = GOSatelliteType.Satellite;
        ...
        private UniTask destroyTilesRoutine;
		...
		void Start () 
	    {
			...
			if (useElevation && !CheckIfLayerMaskExists ("GOTerrain")) {
				Debug.LogError ("[GOMap] GO Map needs a Unity Layer named \"GOTerrain\" to be set up. Open the layers tab in the top right section of your Unity editor.");
				return;
			}
            ...
            {
                Debug.LogWarning("[GOMap] Elevation at zoom level " + zoomLevel + "is not available with this map provider");
                zoomLevel = 14;
                //tileBuffer = tileBuffer == 0 ? 0 : tileBuffer - 1;
            }
			...
			if ( mapType == GOMapType.Nextzen && (nextzen_api_key == null || nextzen_api_key == "")) {
				Debug.LogError ("[GOMap] Nextzen api key is missing, GET iT HERE: https://developers.nextzen.org/keys");
				return;
			}
				...
				Debug.LogError ("[GOMap] Open Street Maps api key is missing, GET IT HERE: https://cloud.maptiler.com"); 
				return;
			}
		...
		public async void OnLocationChanged (Coordinates currentLocation) {

			if (tileBackground != null /*&& Application.isMobilePlatform*/) {
				DestroyTemporaryMapBackground ();
			}

			await ReloadMap (currentLocation,true);
		}

		public async void OnOriginSet (Coordinates currentLocation) {
			...
			await ReloadMap (currentLocation,false);
		}
		...
		public async UniTask ReloadMap (Coordinates location, bool delayed) {
			...
			foreach (GOTile tile in newTiles) {
				tasks.Add(tile.LoadTileData(layers, delayed));
            }
		...
		public async UniTask DestroyTiles (List <Vector2> list) {
		...
		GOTile createSmartTileObject (Vector2 tileCoords, int Zoom) {
            ...
            GOTileObj goTile = new GOTileObj (tileCoords,zoomLevel, mapType,useElevation, elevationMultiplier, new Vector2 (terrainResolution,terrainResolution), useCache, addGoFeatureComponents,locationManager.worldScale,useSatelliteBackground, satelliteType,satellite4X, combineFeatures,navmeshSettings);
...
//			goTile.verticesInWorld = goTile.vertsInWorld (zoomLevel);
		...
		public async UniTask createTileWithPreloadedData(Vector2 tileCoords, int Zoom, bool delayedLoad, object mapData) {
...
//				tile.map = this;
				...
				await tile.ParseTileData(layers,delayedLoad,layerNames());
		...
		public void BuildInsideEditor () {
            ...
            ReloadMap (locationManager.worldOrigin.tileCenter (locationManager.zoomLevel), true);
        ...
        public void BuildMapPortionInsideEditor(Coordinates location, Coordinates origin)
        {
            ...
            ReloadMap(location, true);

        }

        public async UniTask TestEditorUnityWebRequest() {
