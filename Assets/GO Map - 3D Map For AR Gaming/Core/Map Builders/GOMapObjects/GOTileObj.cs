...
namespace GoMap
{
    ...
    public class GOTileObj
    {
        ...
        [HideInInspector] public GoMap.GOMap.GOSatelliteType satelliteType;
        ...
        public GOTileObj(Vector2 tileCoordinates, int zoomLevel, GoMap.GOMap.GOMapType mapType, bool useElevation, float elevationMultiplier, Vector2 resolution,
                          bool useCache, bool addGofeatureComponents, float worldScale, bool useSatelliteBackground, GoMap.GOMap.GOSatelliteType satelliteType, bool satellite4X, bool combineFeatures, GONavMeshSettings navmeshSettings)
        {
        ...
        public GOMesh elevatedTerrainMesh()
        {
            ...
            for (int v = 0, z = 0; z < (int)resolution.y + 1; z++)
            {
                ...
                {
                    float height = 0.0f;
                    Color32 c32 = new Color32(255, 255, 255, 255);
                    if (terrainTex!=null)
                    {
                        c32 = terrainTex.calculateColor(new Vector2(x, z), offset, resolution);
                        height = terrainTex.ConvertColorToAltitude(c32);
        ...
        public async UniTask downloadData(Action completion)
        {
            ...
            foreach (GOTileData data in tileData)
            {
                ...
                switch(data.type)
                {
                    case GOTileData.GODataType.Texture:
                    case GOTileData.GODataType.Satellite:
                    case GOTileData.GODataType.Satellite4X:
                    case GOTileData.GODataType.DEM:
                    case GOTileData.GODataType.Normals:
                        isTexture = true;
                        break;
                    default:
                        break;
                ...
                await data.Download(isTexture, (Texture2D tex, byte[] bytes, string text, string error) =>
                {

                    if (String.IsNullOrEmpty(error))
                    {
                        data.prepareData(tex,bytes);
        ...
        public async UniTask downloadData(MonoBehaviour host)
        {
            ...
            List<UniTask> datatasks = new List<UniTask>();
            foreach (GOTileData data in tileData)
            {
                bool isTexture = false;
                switch (data.type)
                ...
                datatasks.Add(data.Download(isTexture, (Texture2D tex,byte[] bytes, string text, string error) =>
                {
                    if (String.IsNullOrEmpty(error))
                    {
                        data.prepareData(tex,bytes);
            ...
            await UniTask.WhenAll(datatasks);
        ...
        public Texture2D getSatelliteTexture()
        {
            ...
            {
                var data = getSatelliteData();
                if (data == null)
                    return null;
                return data.ToTexture2D();
            }
