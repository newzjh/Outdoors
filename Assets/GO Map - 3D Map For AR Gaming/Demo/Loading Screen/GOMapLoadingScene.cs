...
namespace GoMap
{
    public class GOMapLoadingScene : MonoBehaviour
    {
        ...
        async void OnOriginSet(Coordinates worldOrigin)
        {
            await downloadMapData(worldOrigin);
        }
        #endregion

        #region Download Map Data

        async UniTask downloadMapData(Coordinates worldOrigin)
        {
            ...
            foreach (Vector2 tileCoords in adiacentTiles)
            {
                ...
                //Download tile data
                await tile.goTile.downloadData(this);
        ...
        GOTile createSmartTileObject(Vector2 tileCoords, int Zoom)
        {
            ...
            GOTileObj goTile = new GOTileObj(tileCoords, locationManager.zoomLevel, (GoMap.GOMap.GOMapType)mapType, useElevation, 1, new Vector2(50, 50), useCache, false, locationManager.worldScale, useSatelliteBackground, GoMap.GOMap.GOSatelliteType.Satellite,satellite4X, false, navmeshSettings);
